...
// Increased from 0.05 to 0.08 → "Too Far" triggers at a closer distance
static float AREA_TOO_FAR   = 0.08f * 320 * 240;
...
protected:
    void detect_result_cb(const WhoDetect::result_t &result) override {
        ...
        if (SCREEN_W == 320 && SCREEN_H == 240 && result.img.width > 0) {
            ...
            AREA_TOO_FAR   = 0.08f * SCREEN_W * SCREEN_H;   // increased threshold
        }
...
extern "C" void app_main(void)
{
    ...
    // Camera sensor tuning for better hand detection (especially close hands)
    sensor_t *sensor = esp_camera_sensor_get();
    if (sensor) {
        // Apply 180° rotation (as before)
        if (sensor->set_vflip) sensor->set_vflip(sensor, 1);
        if (sensor->set_hmirror) sensor->set_hmirror(sensor, 1);

        // Additional tuning from reference code to improve hand visibility
        ...
        ESP_LOGI(TAG, "Camera sensor tuned (contrast, brightness, white balance, etc.)");
    } else {
        ESP_LOGW(TAG, "Cannot get camera sensor – tuning not applied");
    }

    // Create and run the application
    auto app = new WhoHandPositionAppLCD(frame_cap);
    auto *model = new HandDetect(
        HandDetect::ESPDET_PICO_224_224_HAND,
        false   // quantization OFF – restart clears any leak
    );
    // Lower threshold to catch more detections (same as reference)
    model->set_score_thr(0.3f);
    app->set_model(model);
